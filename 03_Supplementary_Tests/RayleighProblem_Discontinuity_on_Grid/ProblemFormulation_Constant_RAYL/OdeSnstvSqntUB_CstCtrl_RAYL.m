function dx = OdeSnstvSqntUB_CstCtrl_RAYL(t,x,Dec,Index,N,CurGrd)
    % 参数定义
	...
	dxdpVct = reshape(x(3+1:3+N*3*1),3,1*N).';
	Idx0 = 3 + N*3*1 + 0*(1+1*N);
    		dx(Idx0 + 1 ) = Smoothing_Max_with_0(x(2)/6,0.001);
    		dDotgdp = (1/6)*dxdpVct(:,2) ;
    		dx(Idx0 + (2:1*N+1) ) = Smoothing_Max_with_0_der(x(2)/6,0.001)*dDotgdp;
