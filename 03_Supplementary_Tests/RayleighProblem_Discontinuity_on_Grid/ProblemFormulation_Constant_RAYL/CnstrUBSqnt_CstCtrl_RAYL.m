function [Cieq, Ceq, G_Cieq, G_Ceq] = CnstrUBSqnt_CstCtrl_RAYL(u,CvpGrd,x0,CstrIntvl)
...
for i = 1:NTrGd
	...
	if ~isempty(IdxC_t0) % 如果当前阶段起始点是某个约束区间的网格，更新积分约束初值
		...
		StatAug0(Idx0a+1) = CurDec(1) + StatAug0(1)/6;
    
		GraStatAug0 = reshape(StatAug0(NStat+1:NStat+NStat*NDec),NStat,NDec).';
		StatAug0(Idx0a+2:Idx0a+1+NDec) = GraStatAug0(:,1)/6;
        StatAug0(Idx0a+1+CurIdx) = StatAug0(Idx0a+1+CurIdx) +1;
        % G_c(CurIdx,i) = G_c(CurIdx,i) + 1;
	end

	% 对系统进行积分
	[t, x] = ode45(@(t,x)OdeSnstvSqntUB_CstCtrl_RAYL(t,x,CurDec,CurIdx,N,CurCVPGrd),...
