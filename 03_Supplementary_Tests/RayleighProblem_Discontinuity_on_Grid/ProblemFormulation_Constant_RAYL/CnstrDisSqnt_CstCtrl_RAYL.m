function [Cieq, Ceq, G_Cieq, G_Ceq] = CnstrDisSqnt_CstCtrl_RAYL(u,CvpGrd,x0,T_Dis)
...
% 定义离散路径约束
T_DisPath1 = T_Dis{1};
CieqDisPath1 = zeros(1,numel(T_DisPath1)-1);
G_CieqDisPath1 = zeros(NDec,numel(T_DisPath1)-1);
...
for i = 1:NTrGd
	...
	% 对系统进行积分
	[t, x] = ode45(@(t,x)OdeSnstvSqnt_CstCtrl_RAYL(t,x,CurDec,CurIdx,N,[C_t0,C_tf]),...
	...
	for j=1:numel(T_DisPath1)
		...
		if ( (TD>=TranGrd(i)) && ( (i==N) || (TD<TranGrd(i+1))))==1 % 找出区间中的TD
			...
			CieqDisPath1(1,j) = CurDec(1) + StatTD(1)/6;
			G_CieqDisPath1(:,j) = GraStatTD(:,1)/6;
            G_CieqDisPath1(CurIdx,j) = G_CieqDisPath1(CurIdx,j)+1;
		end
	end
