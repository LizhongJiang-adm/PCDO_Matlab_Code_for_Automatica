function [Cieq, Ceq, G_Cieq, G_Ceq] = CnstrUBSqnt_CstCtrl_JCB(u,CvpGrd,x0,CstrIntvl)
...
for i = 1:NTrGd
	...
	if ~isempty(IdxC_t0) % 如果当前阶段起始点是某个约束区间的网格，更新积分约束初值
		...
		StatAug0(Idx0a+1) = StatAug0(2) - 8*(C_t0 - 1/2)^2 + 1/2;
	...
	% 对系统进行积分
	[t, x] = ode45(@(t,x)OdeSnstvSqntUB_CstCtrl_JCB(t,x,CurDec,CurIdx,N,CurCVPGrd),...
