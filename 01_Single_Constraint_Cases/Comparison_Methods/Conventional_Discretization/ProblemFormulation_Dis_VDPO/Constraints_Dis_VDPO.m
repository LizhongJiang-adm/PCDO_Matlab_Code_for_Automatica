function [Cieq, Ceq, G_Cieq, G_Ceq] = Constraints_Dis_VDPO(u,CvpGrd,x0)
...
CieqDisPath1(1,1) = - StatTD(1) - 2/5;
G_CieqDisPath1(:,1) = - GraStatTD(:,1);
...
for i = 1:NTrGd
	...
	% 对系统进行积分
	[t, x] = ode45(@(t,x)OdeSnstvSqnt_CstCtrl_VDPO(t,x,CurDec,CurIdx,N,[C_t0,C_tf]),...
	...
	StatTD = XendAug; % 插值求得系统状态在TD的值
	GraStatTD = GraXend;
	CieqDisPath1(1,i+1) = - StatTD(1) - 2/5;
	G_CieqDisPath1(:,i+1) = - GraStatTD(:,1);
	...
	% 		CieqDisPath1(1,j) = - StatTD(1) - 2/5;
	% 		G_CieqDisPath1(:,j) = -GraStatTD(:,1);
	% 	end
	% end
