% =========================================================================
% Main script for solving the VDPO Problem with the aBB Method
% =========================================================================
% Description:
% This script sets up the VDPO problem, configures the aBB Method solver,
...
clc;
close all;
addpath("ProblemFormulation_aBB_VDPO");

%% --- 3. Setup Problem and Options ---
fprintf('Step 1: Defining the VDPO problem for the aBB Method...\n');
problem = define_VDPO_problem_aBB();

fprintf('Step 2: Configuring solver options for the aBB Method...\n');
% We can reuse the interval method's options file if the tolerances are similar,
% or create a new one: define_solver_options_aBB().
options = define_solver_options_aBB(); % Assuming a new options file
...
switch ExecutionMode
    case 'Solve'
        ...
        if solution.exitFlag > 0
            % We can likely reuse the same standardized plotting function
    ...
    case 'Benchmark'
        ...
        fprintf('---------------------------------------------------\n');
        fprintf('Benchmark Result for VDPO (aBB Method):\n');
...
fprintf('\n--- VDPO (aBB Method) Workflow Complete ---\n');

if strcmp(ExecutionMode,'Benchmark') 
    save("Result_VDPO_aBB");
end